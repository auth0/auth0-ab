...
Experiments.prototype.findAllByPath = function (path) {
  var experiments = _.filter(this.experiments, function(experiment) {
    return new RegExp(experiment.path).test(path);
  });

  return new Experiments({ experiments: experiments });
};

Experiments.prototype.runAllByPath = function(path) {
  var experimentsCollection = path ? this.findAllByPath(path) : this;
