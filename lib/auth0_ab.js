...
Auth0AB.prototype.onReady = function(cb) {
  if (this.ready) { return cb(this); }

  //Watchdog the ready event
  var watchdog = setTimeout(_.bind(function(){
    this.events.removeAllListeners('error');
    this.events.removeAllListeners('ready');
    cb(this);
  }, this), 1500);

  this.events.once('ready', _.bind(function(){
    clearTimeout(watchdog);
    cb(this);
  }, this));

  this.events.once('error', _.bind(function(){
    clearTimeout(watchdog);
    cb(this);
  }, this));
};
